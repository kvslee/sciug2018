...
try: 
    ...
    cursor.execute(open("./sql/circ_trans.sql", 'r').read(), {"interval": days, "trans_types": trans_types})
...
context = {
    'home_libraries_chart': home_libraries_chart,
    'trans_types_chart': trans_types_chart,
    'ptypes_chart': ptypes_chart,
    'item_locations_chart': item_locations_chart,
    'itypes_chart': itypes_chart,
    'time_chart': time_chart
}

save_file = sys.argv[3] if len(sys.argv) > 3 else 'sierra_embed.html'

create_html('sierra_embed_template.html', save_file, context)